...
These Alert's can be sent only from the server to the client.a kind of alert
...
####User input 1102
当User input Alert发送，client显示内容并让user在文本框中输入的内容。the text string 然后发回给server里的Status。server指示client执行用户交互，通过发送`TEXT INPUT`的Alert。Alert至少包含两个Items：

- 第一个Item包含定义在section10.3中的可选参数。
- diergeItem只有一个Data元素，包含需要向user展示的文本。

The terminal displays the message sent inside the Alert then allows the user to type in a text string. This text string is then sent back to the server in a Status message. 
...
```
####User choice 1103（single-choice）1104（multiple-choice）

当Alert发送，user看到一系列可能的选择。Alert body要包含以下Item：

- 第一个Item包含可选参数。定义于section10中。
- 第二个Item之后一个Data元素，包含内容的标题。
- 从第三个Item起，每个Item只包含一个Data，描述一个可能的选择。这些Item从1开始，Item一定要他们发送的顺序排序。Item应该以他们发送的顺序发送给user。

	- 1103 用户的选择在Status中返回，选择的item在Item中返回，**返回的形式是item相对应的id(从1开始)**。
	- 1104 Alert允许用户选择多个item。这种情况下，Item中有多个items发回,**返回的形式为相对应的item的id(从1开始)**
...
client必须跳过不能处理的可选参数。下面定义了可选参数：

- MINDT（Minimum Display Time）
	这个参数显示用户交互应该给用户显示的最小时间。要确保notification message可读。

- MAXDT （Maximum Display Time）
	参数表示等待用户执行user interaction的时长。如果用户在这个时间内没有执行，这个action应该取消，timeout Stutas或者默认响应包发回给server。MAXDT参数一定是正整数。单位是s。
...
- Clients that support Large Object Handling MUST indicate this by having the value of the `DevDetail/LrgObj` flag set to `true`.
...
- If the size does not match then error `status 424 – “Size mismatch”`. 
- If the recipient detects a new data object or command before the previous item has been completed (by the chunk without the `<MoreData/>` Element), the recipient MUST respond with an `Alert 1225 – “End of Data for chunked object not received”`.
...
- The device information MUST be sent using the `Replace` command in the SyncBody. 
- 4. Client MAY include client-generated alerts such as **Client Event** [REPPRO] or **Generic Alert**.


#####Generic Alert or Client Event ---what's the differences

######Client Event 1224

Informs the server that an event has occurred on the client. Event data MUST be contained in Data element of an Item element.

######Generic Alert 1226

- Generic Alert generates from client,as a kind of ALerts
- Anytime after the Client or Server Initiated Management Alert, the client MAY send a Generic Alert message to the server. 
-  In the case of a relation to a Management Object then the Source and LocURI MUST identify the address to that Management Object.

#####Authentication

This section specifies the rules for how the **OMA-DM Protocol-level** and **the transport level** authentication are used.

######Authentication Challenge

- If the response code to a request (message or command) is `401 (‘Unauthorized’)` or `407 (‘Authentication required’)`, the request requires authentication.

#####Application Layer Authentication

The authentication on the application layer is accomplished by using the Cred element in SyncHdr and the Status command associated with SyncHdr.

######Basic authentication with a challenge

- Server response 401 or 407,the client should resends the Package 1 with Cred in SyncHdr.If successfully,it will response 212.
- The Type in Cred is `syncml:auth-basic`
- `<Data>212</Data> <!-- **Authenticated for session** -->`

######MD5 digest access authentication with a challenge

The difference between basic authentication is `syncml:auth-md5` with `Chal` tag
...
The Alert command is specifically used to convey notifications,for example,a mobile device will use this command to initiate a "client-initiated, management session" with a network server.
